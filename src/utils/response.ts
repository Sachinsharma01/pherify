...
export default {
  ...
  badRequest: (res: Response, message: string, data: {}) => {
    setHeaders(res);
    res.status(400).json({ error: true, message, data });
  },
  notFound: (res: Response, message: string, data: {}) => {
    setHeaders(res);
    res.status(404).json({ error: true, message, data });
  },
  internalServerError: (res: Response, message: string, data: {}) => {
    setHeaders(res);
    res.status(500).json({ error: true, message, data });
  },
  unAuthorized: (res: Response, message: string, data: {}) => {
    setHeaders(res);
